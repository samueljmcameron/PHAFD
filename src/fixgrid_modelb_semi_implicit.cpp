...
FixGridModelBSemiImplicit::FixGridModelBSemiImplicit(PHAFD *phafd) : Fix(phafd) {};



void FixGridModelBSemiImplicit::init(const std::vector<std::string> &v_line,
			 bool add_to_names)
/*
 ...
 */
{
  ...
  while (iarg < v_line.size()) {
    ...
    if (v_line.at(iarg) == "mobility") {
      ...
      iarg += 2;
    } else if (v_line.at(iarg) == "kappa") {
      kappa = std::stod(v_line.at(iarg+1));
      iarg += 2;
    } else {
  ...
  if (kappa < 0) 
    throw std::runtime_error("Error: invalid kappa in fix grid/modelb command");
...
void FixGridModelBSemiImplicit::setup()
{
...
void FixGridModelBSemiImplicit::reset_dt()
{
...
void FixGridModelBSemiImplicit::start_of_step()
{
...
void FixGridModelBSemiImplicit::pre_final_integrate()
{
...
void FixGridModelBSemiImplicit::final_integrate()
{
...
void FixGridModelBSemiImplicit::post_final_integrate(bool invert_fft)
{
  if (invert_fft)
...
void FixGridModelBSemiImplicit::point_update(int i , int j, int k)
{
  ...
  (*grid->ft_phi)(i,j,k)
	...
	+ (*conjugate->ft_array)(i,j,k))*normalization)/(1+mobility*kappa*q2*q2*dt);
...
void FixGridModelBSemiImplicit::origin_update()
{
